...
export function handleRequestError(
  ...
  if (data.error) {
    ...
    if ([environment.oneTimePassword.otpChallengeStatusCode, environment.oneTimePassword.otpInvalidStatusCode].indexOf(Number(data.status)) > -1) {

      return throwError(() => {
        return { statusCode: Number(data.status) }
      });
    }
