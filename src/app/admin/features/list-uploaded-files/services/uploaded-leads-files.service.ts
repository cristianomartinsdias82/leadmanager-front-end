...
export class UploadedLeadsFilesService {
  ...
  private uploadedFilesDataRetrievalSubscription = new BehaviorSubject<boolean>(
    true
  );
  onUploadedFilesDataRetrieve$ =
    this.uploadedFilesDataRetrievalSubscription.asObservable();

  private leadsFileRemoveSuccessSubscription = new Subject<{}>();
  onLeadsFileRemoveSuccessful$ =
    this.leadsFileRemoveSuccessSubscription.asObservable();

  fetch(
    search?: string,
    pagingParameters?: PagingParameters
  ): Observable<ApplicationResponse<PagedList<UploadedFileInfo>>> {
    return this.httpClient
      ...
      .pipe(
        tap((appResponse: ApplicationResponse<PagedList<UploadedFileInfo>>) => {
          this.uploadedFilesDataRetrievalSubscription.next(
            appResponse.data!.itemCount > 0
          );
        })
      );
  }
  ...
  downloadFile(id: string) {
    this.httpClient
      .get(
        `${environment.apiUrl}/${UploadedLeadsFilesService.Endpoint}/uploaded-files/${id}/download`,
    ...
    this.promptService.openYesNoDialog(
      ...
      () =>
        this.httpClient
          .delete(
            `${environment.apiUrl}/${UploadedLeadsFilesService.Endpoint}/uploaded-files`,
            { body: { ids: [{ id: file.id, fileId: file.fileId }] } }
          )
          .subscribe({
